...
"SELECT users.name, SUM(pledges.amount) AS pledge_amount FROM users JOIN pledges ON users.id = pledges.user_id GROUP BY users.name ORDER BY pledge_amount, users.name;"
end
